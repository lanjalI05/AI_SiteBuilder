...
const MyProjects = () => {
  ...
  const deleteProject = async (projectId: string) => {
    try {
      ...
      if(!confirm) return
      const { data } = await api.delete(`/api/user/project/${projectId}`)
            ...
            <div className="flex flex-wrap gap-3.5">
              {
                projects.map((project) => (
                  <div onClick={() => navigate(`/projects/${project.id}`)} key={project.id} className="relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300">
                    ...
                    <div className="p-4 text-white bg-linear-180 from-trasparent group-hover:from-indigo-950 to-trasparent transition-colors">
