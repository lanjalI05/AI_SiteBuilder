...
const Projects = () => {
  ...
  const [device, setIsDevice] = useState<"phone" | "tablet" | "desktop">(
    "desktop"
  );
...
// userAuthentication funciton to view project
  ...
  return project ? (
      <div className="flex flex-col h-screen w-full bg-gray-900 text-white">
        <div className="flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar">
          <div className="flex items-center gap-2 sm:min-w-90 text-nowrap">
            ...
            <div className="sm:hidden flex-1 flex justify-end">
              {isMenu ? <MessageSquareIcon onClick={() => setIsMenu(false)} className="size-6 cursor-pointer" /> : <XIcon onClick={() => setIsMenu(true)} className="size-6 cursor-pointer" />}
            </div>
          </div>
          <div className="hidden sm:flex gap-2 bg-gray-950 p-1.5 rounded-md">
            <SmartphoneIcon onClick={() => setIsDevice('phone')} className={`size-6 p-1 rounded cursor-pointer ${device === 'phone' ? "bg-gray-700" : ""}`}/>
            <TabletIcon onClick={() => setIsDevice('tablet')} className={`size-6 p-1 rounded cursor-pointer ${device === 'tablet' ? "bg-gray-700" : ""}`}/>
            <LaptopIcon onClick={() => setIsDevice('desktop')} className={`size-6 p-1 rounded cursor-pointer ${device === 'desktop' ? "bg-gray-700" : ""}`}/>
          </div>
        ...
        <div className="flex-1 flex overflow-auto">
          <Sidebar isMenuOpen={isMenu} project={project} setProject={(p) => setProject(p)} isGenerating={isGenerate} setIsGenerating={setIsGenerate}/>
          <div className="flex-1 p-2 pl-0"><ProjectPreview ref={previewRef} project={project} isGenerating={isGenerate} device={device}/></div>
        </div>
      </div>
  ) : (
    <div className="flex items-center justify-center h-screen">
